...
static void action_callback(const char* action_name, cdsl_arg_node_t* args, void* user_data) {
    ...
    for (cdsl_arg_node_t* a = args; a; a = a->next) {
        ...
        if (a->expr) {
            switch (a->expr->type) {
                case CDSL_EXPR_INT:    printf("%d", a->expr->data.int_val); break;
                case CDSL_EXPR_FLOAT:  printf("%.2f", a->expr->data.float_val); break;
                case CDSL_EXPR_BOOL:   printf("%s", a->expr->data.bool_val ? "true" : "false"); break;
                case CDSL_EXPR_STRING: printf("\"%s\"", a->expr->data.string_val); break;
...
static void demo_supplier_audit(cdsl_schema_t* schema, cdsl_ai_config_t* ai_cfg) {
    print_separator("=== DEMO 1: Supplier Qualification Audit ===");

    const char* nl = "supplier qualification audit with blacklist check and capital verification";
    ...
    printf("  Reason: %s\n", review->reason);
    if (review->suggestions[0]) printf("  Suggestions: %s\n", review->suggestions);
    ...
    typedef struct { const char* name; int cap; int years; int blacklisted; } test_case_t;
    test_case_t cases[] = {
...
static void demo_doc_audit(cdsl_schema_t* schema, cdsl_ai_config_t* ai_cfg) {
    ...
    cdsl_rule_t* rule = cdsl_parse_string(dsl_code);
    if (!rule) { printf("Parse failed.\n"); cdsl_ai_review_free(review); free(dsl_code); return; }

    char err[512] = {0};
    if (!cdsl_verify_rule(rule, schema, err, sizeof(err))) {
        printf("Verify failed: %s\n", err);
        cdsl_free_rule(rule); cdsl_ai_review_free(review); free(dsl_code);
        return;
    }
    ...
    struct { const char* name; const char* fmt; double size; int sig; } cases[] = {
        {"Valid PDF",      "pdf",  5.0, 1},
...
static void demo_content_audit(cdsl_schema_t* schema, cdsl_ai_config_t* ai_cfg) {
    ...
    struct { const char* name; int sens; int pii; double spam; } cases[] = {
...
static void demo_json_context(void) {
    ...
    if (!cdsl_verify_rule(rule, schema, err, sizeof(err))) {
        printf("Verify failed: %s\n", err);
        cdsl_free_rule(rule); cdsl_schema_free(schema);
        return;
    }
