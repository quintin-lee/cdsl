...
static void test_action_cb(const char* action_name, cdsl_arg_node_t* args, void* ud) {
    action_called = 1;
    strncpy(last_action_name, action_name, sizeof(last_action_name) - 1);
}

static cdsl_schema_t* make_test_schema(void) {
    ...
    cdsl_schema_register_action(s, "fail_metric", CDSL_TYPE_VOID, 2, CDSL_TYPE_INT, CDSL_TYPE_STRING);
    return s;
}

void test_context_set_get(void) {
    ...
    while (e) {
        if (strcmp(e->name, "user.age") == 0 && e->value.type == CDSL_TYPE_INT && e->value.data.int_val == 25)
            found_age = 1;
        if (strcmp(e->name, "user.name") == 0 && e->value.type == CDSL_TYPE_STRING && strcmp(e->value.data.string_val, "Alice") == 0)
            found_name = 1;
        if (strcmp(e->name, "user.active") == 0 && e->value.type == CDSL_TYPE_BOOL && e->value.data.bool_val == 1)
            found_active = 1;
        e = e->next;
    }
...
void test_context_load_json(void) {
    TEST_BEGIN("context load from JSON");
    cdsl_schema_t* schema = make_test_schema();
    cdsl_context_t* ctx = cdsl_context_create(schema);
    int ok = cdsl_context_load_json(ctx, "{\"user\":{\"age\":30,\"name\":\"Bob\",\"active\":false}}");
    TEST_ASSERT_INT(ok, 1, "json load ok");

    int found = 0;
    for (cdsl_context_entry_t* e = ctx->entries; e; e = e->next) {
        if (strcmp(e->name, "user.age") == 0 && e->value.data.int_val == 30) found = 1;
    }
...
void test_simple_rule_pass(void) {
    TEST_BEGIN("simple rule WHEN false -> PASSED");
    cdsl_schema_t* schema = make_test_schema();
    cdsl_rule_t* rule = cdsl_parse_string(
        "RULE r1 { META { description = \"test\" } WHEN user.age > 100 THEN block(\"too_old\") }");
...
void test_simple_rule_fail(void) {
...
void test_metric_rule_scoring(void) {
    TEST_BEGIN("metric rule scoring");
    cdsl_schema_t* schema = make_test_schema();
    cdsl_rule_t* rule = cdsl_parse_string(
        "RULE scoring {"
        "  META { description = \"score test\" pass_threshold = \"80\" partial_threshold = \"50\" }"
        "  METRIC m1 {"
...
void test_metric_rule_partial(void) {
...
void test_critical_metric_veto(void) {
    TEST_BEGIN("critical metric veto -> FAILED");
    cdsl_schema_t* schema = make_test_schema();
    cdsl_rule_t* rule = cdsl_parse_string(
        "RULE veto {"
        "  META { description = \"veto test\" pass_threshold = \"50\" partial_threshold = \"20\" }"
        "  METRIC m1 {"
...
void test_string_comparison(void) {
    ...
    cdsl_schema_t* schema = make_test_schema();
    cdsl_rule_t* rule = cdsl_parse_string(
        "RULE strcmp { META { desc = \"str\" } WHEN user.name == \"Alice\" THEN block(\"match\") }");
    cdsl_vm_t* vm = cdsl_vm_create(schema);
    cdsl_vm_register_action(vm, "block", test_action_cb);
...
void test_ruleset_batch(void) {
    ...
    cdsl_rule_t* r2 = cdsl_parse_string(
        "RULE b { META { d = \"b\" } WHEN user.age < 0 THEN block(\"b\") }");
...
void test_verify_detailed(void) {
