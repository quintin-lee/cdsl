...
test_builtin_contains(void)
{
	...
	cdsl_rule_t* rule = cdsl_parse_string(
	    "RULE test { WHEN contains(\"hello world\", \"world\") THEN block(\"ok\") }");
	cdsl_rule_report_t* rpt = cdsl_vm_execute(vm, rule, ctx);
	TEST_ASSERT_INT(
...
test_builtin_string_utils(void)
{
	...
	cdsl_rule_t* r_upper = cdsl_parse_string(
	    "RULE r { WHEN uppercase(\"hello\") == \"HELLO\" THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_lower = cdsl_parse_string(
	    "RULE r { WHEN lowercase(\"HELLO\") == \"hello\" THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_trim =
	    cdsl_parse_string("RULE r { WHEN trim(\"  abc  \") == \"abc\" THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_sw = cdsl_parse_string(
	    "RULE r { WHEN startswith(\"hello world\", \"hello\") THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_ew = cdsl_parse_string(
	    "RULE r { WHEN endswith(\"hello world\", \"world\") THEN block(\"ok\") }");
...
test_builtin_misc(void)
{
	...
	cdsl_rule_t* r_typeof =
	    cdsl_parse_string("RULE r { WHEN typeof(\"hello\") == \"STRING\" THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_typeof_int =
	    cdsl_parse_string("RULE r { WHEN typeof(42) == \"INT\" THEN block(\"ok\") }");
...
test_builtin_array(void)
{
	...
	cdsl_rule_t* r_count =
	    cdsl_parse_string("RULE r { WHEN count([1, 2, 3]) == 3 THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_sum =
	    cdsl_parse_string("RULE r { WHEN sum([1, 2, 3]) == 6 THEN block(\"ok\") }");
	...
	cdsl_rule_t* r_typeof =
	    cdsl_parse_string("RULE r { WHEN typeof([1, 2]) == \"ARRAY\" THEN block(\"ok\") }");
