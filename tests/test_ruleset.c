...
static void test_ruleset_basic_creation() {
...
static void test_ruleset_add_remove() {
...
static void test_ruleset_priority_ordering() {
    ...
    TEST_ASSERT(strcmp(it->rule->name, "rule2") == 0, "First rule should be rule2 (priority 1)");
    it = it->next;
    TEST_ASSERT_NOT_NULL(it, "Ruleset should have second entry");
    TEST_ASSERT(strcmp(it->rule->name, "rule3") == 0, "Second rule should be rule3 (priority 5)");
    it = it->next;
    TEST_ASSERT_NOT_NULL(it, "Ruleset should have third entry");
    TEST_ASSERT(strcmp(it->rule->name, "rule1") == 0, "Third rule should be rule1 (priority 10)");
...
static void test_ruleset_batch_execution() {
...
static void test_ruleset_parallel_execution() {
...
static void test_ruleset_hot_reload() {
    ...
    int reloaded = cdsl_ruleset_reload_file(ruleset, "initial_rule", reload_dsl, schema, err, sizeof(err));
    ...
    TEST_ASSERT(strcmp(ruleset->entries->rule->name, "reloaded_rule") == 0, "Rule should be reloaded");
...
static void test_ruleset_dependency_management() {
    ...
    const char* rule1_dsl = "RULE rule1 { META { description = \"First rule\" } WHEN user.age > 0 THEN block(\"test1\") }";
    const char* rule2_dsl = "RULE rule2 { META { description = \"Second rule\" depends_on = \"rule1\" } WHEN user.age > 10 THEN block(\"test2\") }";
    const char* rule3_dsl = "RULE rule3 { META { description = \"Third rule\" depends_on = \"rule2,rule1\" } WHEN user.age > 20 THEN block(\"test3\") }";
...
static void test_ruleset_error_handling() {
    ...
    int reloaded = cdsl_ruleset_reload_file(ruleset, "nonexistent", "RULE test { WHEN x > 0 THEN block(\"test\") }", schema, err, sizeof(err));
...
static void test_complex_ruleset() {
...
static void test_ruleset_performance() {
