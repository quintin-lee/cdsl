...
static const char* kind_str(cdsl_error_kind_t kind) {
    switch (kind) {
        case CDSL_ERR_SYNTAX:  return "SYNTAX";
        case CDSL_ERR_TYPE:    return "TYPE";
        case CDSL_ERR_SEMANTIC:return "SEMANTIC";
        case CDSL_ERR_RUNTIME: return "RUNTIME";
...
void cdsl_error_print(const cdsl_error_t* err) {
    ...
    fprintf(stderr, "[%s] line %d, col %d: %s",
            kind_str(err->kind), err->line, err->column, err->message);
    if (err->hint) fprintf(stderr, "\n  hint: %s", err->hint);
    fprintf(stderr, "\n");
}
