...
static cdsl_json_value_t* parse_object(json_parser_t* p) {
    if (p->src[p->pos] != '{') return NULL;
    p->pos++;
...
static cdsl_json_value_t* parse_array(json_parser_t* p) {
    if (p->src[p->pos] != '[') return NULL;
    p->pos++;
    cdsl_json_value_t* head = NULL;
    cdsl_json_value_t* tail = NULL;
...
static cdsl_json_value_t* parse_value(json_parser_t* p) {
    ...
    if (c == 't' || c == 'f') {
        ...
        if (strncmp(p->src + p->pos, "true", 4) == 0) {
            v->value.bool_val = 1; p->pos += 4;
        } else {
            v->value.bool_val = 0; p->pos += 5;
        }
        return v;
    }
    if (c == 'n') {
...
void cdsl_json_free(cdsl_json_value_t* val) {
    ...
    switch (val->type) {
        case JSON_STRING: free(val->value.string_val); break;
        case JSON_OBJECT:
        case JSON_ARRAY: {
            cdsl_json_value_t* item = val->type == JSON_OBJECT ?
                val->value.object.items : val->value.array.items;
            while (item) {
